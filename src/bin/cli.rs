...
use brewsty::domain::entities::{PackageType, PackageCategory};
...
async fn main() {
    ...
    match cli.command {
        Commands::List { package_type, search } => {
            ...
            match container.list_installed.execute(pt).await {
                Ok(packages) => {
                    ...
                    println!("{}", formatter.format_message(&format!("Found {} packages:", filtered.len())));
                    for pkg in filtered {
                ...
                Err(e) => {
                    println!("{}", formatter.format_error(&format!("Failed to list packages: {}", e)));
                }
            }
        }
        Commands::Search { query, package_type } => {
            let pt = parse_package_type_arg(&package_type);
            match container.search.execute(&query, pt).await {
                Ok(packages) => {
                    println!("{}", formatter.format_message(&format!("Search results for '{}':", query)));
                ...
                Err(e) => {
                    println!("{}", formatter.format_error(&format!("Search failed: {}", e)));
                }
            }
        }
        Commands::Install { name, version } => {
            ...
            match container.install.execute(pkg).await {
                Ok(_) => {
                    println!("{}", formatter.format_message("Package installed successfully!"));
                }
                Err(e) => {
                    println!("{}", formatter.format_error(&format!("Installation failed: {}", e)));
                }
            }
        }
        Commands::Uninstall { name, force } => {
            ...
            match container.uninstall.execute(pkg).await {
                Ok(_) => {
                    println!("{}", formatter.format_message("Package uninstalled successfully!"));
                }
                Err(e) => {
                    println!("{}", formatter.format_error(&format!("Uninstallation failed: {}", e)));
                }
            }
        }
            ...
            match name {
                Some(pkg_name) => {
                    ...
                    match container.update.execute(&pkg).await {
                        Ok(_) => {
                            println!("{}", formatter.format_message("Package updated successfully!"));
                        }
                        Err(e) => {
                ...
                None => {
                    match container.update_all.execute().await {
                        Ok(_) => {
                            println!("{}", formatter.format_message("All packages updated successfully!"));
                        }
                        Err(e) => {
                            println!("{}", formatter.format_error(&format!("Update all failed: {}", e)));
                        }
                    }
                }
            }
        ...
        Commands::Info { name } => {
            match container.get_package_info.execute(&name, PackageType::Formula).await {
                Ok(pkg) => {
                    println!("{}", formatter.format_message(&format!("Package: {}", pkg.name)));
                    println!("  Version: {}", pkg.version.unwrap_or_else(|| "unknown".to_string()));
                ...
                Err(e) => {
                    println!("{}", formatter.format_error(&format!("Package info not found: {}", e)));
                }
            }
        }
        Commands::Orphans { remove } => {
            match container.clean_orphans.preview().await {
                Ok(preview) => {
                    println!("{}", formatter.format_message(&format!("Found {} orphan packages:", preview.items.len())));
                    for item in &preview.items {
                        println!("  • {} ({})", item.path, format_bytes(item.size));
                    }
                    println!("\nTotal space to reclaim: {}", format_bytes(preview.total_size));
                    
                    if remove {
                        match container.clean_orphans.execute().await {
                            Ok(_) => {
                                println!("{}", formatter.format_message("Orphans removed successfully!"));
                            }
                            Err(e) => {
                                println!("{}", formatter.format_error(&format!("Failed to remove orphans: {}", e)));
                            }
                        }
                    } else {
                ...
                Err(e) => {
                    println!("{}", formatter.format_error(&format!("Failed to detect orphans: {}", e)));
                }
            }
        }
        Commands::Stats => {
            // Get installed packages count
            let formulae = container.list_installed.execute(PackageType::Formula).await.unwrap_or_default();
            let casks = container.list_installed.execute(PackageType::Cask).await.unwrap_or_default();
            ...
            // Count outdated
            let outdated_formulae = container.list_outdated.execute(PackageType::Formula).await.unwrap_or_default();
            let outdated_casks = container.list_outdated.execute(PackageType::Cask).await.unwrap_or_default();
            println!("  Outdated: {}", outdated_formulae.len() + outdated_casks.len());
        }
        Commands::Export { output, format } => {
            let packages = container.list_installed.execute(PackageType::Formula).await.unwrap_or_default();
            let casks = container.list_installed.execute(PackageType::Cask).await.unwrap_or_default();
            ...
            match export_packages_to_json(&all_packages, path) {
                Ok(_) => {
                    println!("{}", formatter.format_message(&format!("Exported {} packages to {}", all_packages.len(), output_path)));
                }
                Err(e) => {
                    println!("{}", formatter.format_error(&format!("Export failed: {}", e)));
                }
            }
        ...
        Commands::Import { input, install } => {
            ...
            match import_packages(path) {
                Ok(packages) => {
                    println!("{}", formatter.format_message(&format!("Imported {} packages from {}", packages.len(), input)));
                    ...
                    if install {
                        ...
                        println!("{}", formatter.format_message(&format!("Installed: {}, Failed: {}", success, failed)));
                    } else {
                        println!("{}", formatter.format_message("Dry run complete. Use --install to actually install packages."));
                    }
                }
                Err(e) => {
                    println!("{}", formatter.format_error(&format!("Import failed: {}", e)));
                }
            }
        }
    }
